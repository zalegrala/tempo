...
func (i *instance) Search(ctx context.Context, req *tempopb.SearchRequest) (*tempopb.SearchResponse, error) {
	ctx, span := i.tracer.Start(ctx, "instance.Search")
	defer span.End()
	...
	search := func(blockID uuid.UUID, block common.Searcher, spanName string) {
		ctx, span := i.tracer.Start(ctx, "instance.searchBlock."+spanName)
		defer span.End()
...
func (i *instance) SearchTags(ctx context.Context, scope string) (*tempopb.SearchTagsResponse, error) {
	ctx, span := i.tracer.Start(ctx, "instance.SearchTags")
	defer span.End()
	...
	search := func(ctx context.Context, s common.Searcher, dv *util.DistinctStringCollector, spanName string) error {
		ctx, span := i.tracer.Start(ctx, "instance.SearchTags."+spanName)
		defer span.End()
...
func (i *instance) SearchTagsV2(ctx context.Context, scope string) (*tempopb.SearchTagsV2Response, error) {
	ctx, span := i.tracer.Start(ctx, "Ingester.SearchTagsV2")
	defer span.End()
...
func (i *instance) SearchTagValuesV2(ctx context.Context, req *tempopb.SearchTagValuesRequest) (*tempopb.SearchTagValuesV2Response, error) {
	...
	ctx, span := i.tracer.Start(ctx, "instance.SearchTagValuesV2")
	defer span.End()
	...
	if i.headBlock != nil {
		wg.Add(1)
		go func() {
			ctx, span := i.tracer.Start(ctx, "instance.SearchTagValuesV2.headBlock")
			defer span.End()
	...
	for _, b := range i.completeBlocks {
		wg.Add(1)
		go func(b *localBlock) {
			ctx, span := i.tracer.Start(ctx, "instance.SearchTagValuesV2.completedBlock")
			defer span.End()
			defer wg.Done()
	...
	for _, b := range i.completingBlocks {
		wg.Add(1)
		go func(b common.WALBlock) {
			ctx, span := i.tracer.Start(ctx, "instance.SearchTagValuesV2.completingBlock")
			defer span.End()
			defer wg.Done()
			if err := searchBlock(ctx, b); err != nil {
