...
func (h *HistogramAggregator) Results() SeriesSet {
	...
	for _, in := range h.ss {
		...
		for qIdx, q := range h.qs {
			...
			// Track which buckets are used for this quantile across time intervals
			quantileBuckets := make([]float64, len(in.hist))
			...
			for i := range in.hist {
				...
				if len(buckets) == 0 {
					quantileBuckets[i] = math.NaN()
					continue
				}

				quantileValue, bucketMax := Log2QuantileWithBucket(q, buckets)
				ts.Values[i] = quantileValue
				quantileBuckets[i] = bucketMax
			}

			// Simple exemplar distribution - just divide exemplars evenly across quantiles
			if len(in.exemplars) > 0 {
				ts.Exemplars = h.selectExemplarsFromQuantileBuckets(in.exemplars, quantileBuckets, qIdx)
			}
...
// selectExemplarsFromQuantileBuckets selects exemplars for this quantile series.
// Conservative approach: just distribute available exemplars evenly across quantiles.
func (h *HistogramAggregator) selectExemplarsFromQuantileBuckets(allExemplars []Exemplar, quantileBuckets []float64, quantileIdx int) []Exemplar {
	if len(allExemplars) == 0 {
		return nil
	}

	// Simple round-robin distribution across quantiles
	totalQuantiles := len(h.qs)
	var result []Exemplar

	// Start from this quantile's offset and take every Nth exemplar
...
// Log2QuantileWithBucket returns both the quantile value and the bucket information
// where the quantile was found. This is useful for selecting appropriate exemplars.
func Log2QuantileWithBucket(p float64, buckets []HistogramBucket) (float64, float64) {
	if math.IsNaN(p) ||
		p < 0 ||
		p > 1 ||
		len(buckets) == 0 {
	...
	// Return both the calculated quantile value and the bucket max it came from
	return mid, buckets[bucket].Max
}
